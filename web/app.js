...
function updateSearchBtnCount() {
    ...
    document.querySelectorAll('[data-search-btn]').forEach(b => { b.textContent = label; });
}

let _countTimer = null, _countSeq = 0;
function scheduleLiveCount() { clearTimeout(_countTimer); _countTimer = setTimeout(fetchLiveCount, 400); }
async function fetchLiveCount() {
    const payload = buildSearchPayload();
    delete payload.page; delete payload.sort;
...
window.toggleLangMenu = function (e) {
    ...
    if (willOpen) { const active = pop.querySelector('.lang-opt.is-active') || pop.querySelector('.lang-opt'); if (active) active.focus(); }
};
function _closeLangMenu(focusTrigger) {
    const pop = document.querySelector('#lang-menu .lang-pop');
    const trig = document.querySelector('#lang-menu .lang-trigger');
    if (pop && !pop.hidden) pop.setAttribute('hidden', '');
    if (trig) { trig.setAttribute('aria-expanded', 'false'); if (focusTrigger) trig.focus(); }
}
...
function buildSearchPayload() {
    ...
    const MULTI = {
        'f-brand': 'manufacturers', 'f-model': 'models',
        'f-body': 'body_types', 'f-fuel': 'fuels',
        'f-gearbox': 'gearboxes', 'f-drive': 'drives', 'f-location': 'locations',
    };
...
(function populateYears() {
    ...
    const fixRange = (changed) => {
        const f = parseInt(yFrom.value, 10), t = parseInt(yTo.value, 10);
...
(function wireNumRanges() {
    ...
    for (const [a, b] of [['f-price-from', 'f-price-to'], ['f-mileage-from', 'f-mileage-to']]) {
        const from = document.getElementById(a), to = document.getElementById(b);
...
function _fmtKm(n) { return n >= MILEAGE_MAX ? '1,000,000+' : Number(n).toLocaleString(); }
function initMileageSlider() {
    ...
    const apply = (which) => {
        ...
        if (a > b) { if (which === 'from') { a = b; from.value = a; } else { b = a; to.value = b; } }
    ...
    window._resetMileage = () => { from.value = 0; to.value = MILEAGE_MAX; apply('init'); };
    apply('init');
}
...
window.toggleSidebar = function () {
    ...
    if (window.matchMedia('(max-width: 860px)').matches) { open ? lockScroll('sheet') : unlockScroll('sheet'); }
...
function updateFilterResets() {
    for (const [kind, ids] of Object.entries(_RESET_GROUPS)) {
        ...
        const hasVal = ids.some((id) => {
            ...
            if (!el) return false;
            if (el.multiple) {
                // a subset narrows; none or all selected both mean "all" (no filter)
...
async function loadMakes() {
    ...
    Array.from(brand.options).forEach(o => { o.selected = !!o.value && (firstFill || keep.has(o.value)); });
...
const _BRAND_COLORS = {
    'BMW':'#0066B1','Mercedes-Benz':'#2B2B2B','Toyota':'#D81E2C','Audi':'#A50E26',
    'Volkswagen':'#00237A','Honda':'#C4122F','Ford':'#1B3F8B','Hyundai':'#002C5F',
    'Nissan':'#B3002A','Lexus':'#2A2A2A','Kia':'#0B141B','Mazda':'#1A1A1A',
    'Porsche':'#8E2420','Chevrolet':'#A37B2C','Mitsubishi':'#C40010','Subaru':'#013C74',
    'Volvo':'#003057','Land Rover':'#00592C','Jeep':'#16321F','Tesla':'#B00000',
    'Opel':'#2B2B2B','Renault':'#1A2E4A','Peugeot':'#15244A','Fiat':'#7A1626',
    'Suzuki':'#C4051A','Skoda':'#0E3A2F','Mini':'#1A1A1A','Jaguar':'#14361F',
    'Bentley':'#14342A','Maserati':'#14233F','Cadillac':'#7E1A2A','Acura':'#2A2A2A',
    'Infiniti':'#2A2A2A','Genesis':'#2A2A2A','Dodge':'#8E1520','GMC':'#8E1520','Chrysler':'#14233F',
};
...
function _brandLogoSlug(name) { return name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''); }
...
const _LOC_SEP = '\u001F';
const _LOC_COUNTRY_SHORT = { 'არაბეთის გაერთიანებული საემიროები': 'ემირატები' };
...
function _seriesOf(brand, model) {
    const b = brand.toLowerCase(); const m = model.trim();
    if (b === 'bmw') {
...
function _buildModelOptions(brands, keep) {
    ...
    for (const b of brands) {
        ...
        for (const m of (_makes[b] || [])) {
            ...
            if (s) { if (!groups.has(s)) groups.set(s, []); groups.get(s).push(m); }
            else flat.push(m);
        }
        for (const [series, ms] of [...groups.entries()]) {
            if (ms.length < 2) { flat.push(...ms); groups.delete(series); }
        }
        ...
        if (groups.size) {
            ...
            if (flat.length) { out.push(`<option disabled value="" data-n="${flat.length}"${bAttr}>${escapeHtml(prefix + t('other'))}</option>`); flat.forEach(m => out.push(opt(m))); }
        } else {
...
function onBrandChange() {
    ...
    // models only mean something once you narrow to a handful of brands; "all brands"
    // (== no brand filter) or a near-complete set leaves the model dropdown empty/disabled
...
const _URL_LISTS = { brand: 'manufacturers', model: 'models', body: 'body_types', fuel: 'fuels', gear: 'gearboxes', drive: 'drives', loc: 'locations' };
const _URL_NUMS = { yf: 'year_from', yt: 'year_to', pf: 'price_from', pt: 'price_to', mf: 'mileage_from', mt: 'mileage_to' };
function payloadToParams(p) {
    const sp = new URLSearchParams();
    if (p.query) sp.set('q', p.query);
    for (const [k, f] of Object.entries(_URL_LISTS)) if (p[f] && p[f].length) sp.set(k, p[f].join('|'));
    for (const [k, f] of Object.entries(_URL_NUMS)) if (p[f] != null) sp.set(k, p[f]);
...
async function doSearch(opts = {}) {
    ...
    if (_hit && Date.now() - _hit.t < _SEARCH_TTL) {
        const cached = { ..._hit.data, remaining_searches: null };
        lastResponse = cached;
        document.body.classList.remove('mode-detail');
    document.body.classList.add('mode-results');
        document.getElementById('empty-state').classList.add('hidden');
    ...
    try {
        ...
        setOffline(false);
        _searchCache.set(_ckey, { t: Date.now(), data: lastResponse });
        renderResults(lastResponse);
    } catch (e) {
        const offline = !navigator.onLine || e instanceof TypeError;
        if (offline) setOffline(true);
        results.innerHTML = '';
        showSearchError(meta, offline ? t('offline_msg') : t('err_fetch', { msg: e.message }));
    } finally {
...
function _driveIcon(v) {
    const s = (v || '').toLowerCase();
    const on = '', off = ' opacity=".35"';
    let tl = on, tr = on, bl = on, br = on;
    if (/4x4|4wd|awd|ოთხ|სრულ|all|სავ/.test(s)) { /* ყველა მუქი */ }
    else if (/უკან|rear|\brwd\b|უკანა/.test(s)) { tl = off; tr = off; }
    else { bl = off; br = off; } // წინა (default)
...
const _CYR = { 'а':'a','б':'b','в':'v','г':'g','д':'d','е':'e','ё':'e','ж':'zh','з':'z','и':'i','й':'i','к':'k','л':'l','м':'m','н':'n','о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'h','ц':'c','ч':'ch','ш':'sh','щ':'sch','ъ':'','ы':'i','ь':'','э':'e','ю':'iu','я':'ia' };
const _GEO = { 'ა':'a','ბ':'b','გ':'g','დ':'d','ე':'e','ვ':'v','ზ':'z','თ':'t','ი':'i','კ':'k','ლ':'l','მ':'m','ნ':'n','ო':'o','პ':'p','ჟ':'zh','რ':'r','ს':'s','ტ':'t','უ':'u','ფ':'p','ქ':'k','ღ':'gh','ყ':'k','შ':'sh','ჩ':'ch','ც':'c','ძ':'dz','წ':'c','ჭ':'ch','ხ':'h','ჯ':'j','ჰ':'h' };
...
function getHiddenList() { try { return JSON.parse(localStorage.getItem(HIDDEN_KEY) || '[]').map(x => typeof x === 'string' ? { key: x } : x); } catch { return []; } }
function setHiddenList(list) { try { localStorage.setItem(HIDDEN_KEY, JSON.stringify(list)); } catch {} }
function getHidden() { return new Set(getHiddenList().map(x => x.key)); }   // key set, used to filter results

window.hideCar = function (key, btn) {
    const list = getHiddenList();
    if (!list.some(x => x.key === key)) {
        const d = _carData[key] || {};
        list.push({ key, title: d.title || key, thumb: d.thumb || '' });
        setHiddenList(list);
    }
...
function showToast(msg, actionLabel, actionFn) {
    ...
    if (actionLabel) { const b = el.querySelector('button'); b.textContent = actionLabel; b.onclick = () => { if (actionFn) actionFn(); hideToast(); }; }
...
const _carFull = {};
function getCompare() { try { const v = JSON.parse(localStorage.getItem(COMPARE_KEY) || '[]'); return Array.isArray(v) ? v : []; } catch { return []; } }
function setCompare(list) { try { localStorage.setItem(COMPARE_KEY, JSON.stringify(list)); } catch {} }
function isInCompare(key) { return getCompare().some(c => c.key === key); }
function summarizeCar(car) {
    return {
        key: `${car.source}-${car.source_id}`,
        title: carTitle(car),
        price: car.price_amount, currency: car.price_currency,
        year: car.year, mileage: car.mileage_km,
        engine: car.engine_volume_l, power: car.power_hp,
        fuel: car.engine_type, gearbox: car.gearbox, drive: car.drive_wheels,
        body: car.body_type, location: car.location, customs: car.customs_cleared,
...
window.toggleCompare = function (carKey, btn) {
    ...
    if (idx >= 0) { list.splice(idx, 1); if (btn) { btn.classList.remove('on'); btn.setAttribute('aria-pressed', 'false'); } }
    else {
        if (list.length >= 4) { showToast(t('compare_max')); return; }
        const car = _carFull[carKey];
        if (!car) return;
        list.push(summarizeCar(car));
        if (btn) { btn.classList.add('on'); btn.setAttribute('aria-pressed', 'true'); btn.classList.remove('pop'); void btn.offsetWidth; btn.classList.add('pop'); }
    }
    setCompare(list);
    renderCompareBar();
};
...
window.clearCompare = function () {
    setCompare([]);
    renderCompareBar();
    document.querySelectorAll('.rcard-compare.on').forEach(b => { b.classList.remove('on'); b.setAttribute('aria-pressed', 'false'); });
    closeCompare();
};
window.removeFromCompare = function (key) {
    ...
    renderCompareBar();
    if (list.length) renderCompareModal(); else closeCompare();
};
const CMPSAVE_KEY = 'cdb_compsaves';
function getCompSaves() { try { const v = JSON.parse(localStorage.getItem(CMPSAVE_KEY) || '[]'); return Array.isArray(v) ? v : []; } catch { return []; } }
function setCompSaves(l) { try { localStorage.setItem(CMPSAVE_KEY, JSON.stringify(l)); } catch {} }
function _cmpSetKey(cars) { return (cars || []).map(c => c.key).slice().sort().join('|'); }
function _isComparisonSaved(cars) { const k = _cmpSetKey(cars); return getCompSaves().some(s => _cmpSetKey(s.cars) === k); }
...
window.saveComparison = function () {
    ...
    if (_isComparisonSaved(list)) { _syncCmpSaveBtn(); return; }   // no duplicate saves
    setCompSaves([{ cars: list }, ...getCompSaves()].slice(0, 12));
    updateSavedCount();
    showToast(t('saved_done'));
    _syncCmpSaveBtn();
};
window.loadComparison = function (i) {
    const s = getCompSaves()[i]; if (!s) return;
    setCompare(s.cars);
    closeSaved();
...
window.deleteComparison = function (i) {
    const saves = getCompSaves(); saves.splice(i, 1); setCompSaves(saves);
...
const _CMP_FACTORS = [
    { weight: 3, better: 'high', ref: 8,      val: c => c.year || null },
    { weight: 3, better: 'low',  ref: 150000, val: c => c.mileage || null },
    { weight: 3, better: 'low',  ref: 'mean', val: c => c.price > 0 ? c.price : null },
    { weight: 1, better: 'high', ref: 80,     val: c => c.power || null },
    { weight: 1, better: 'high', ref: 1,      val: c => c.drive ? _driveScore(c.drive) : null },
    { weight: 1, better: 'high', ref: 1,      val: c => c.customs === true ? 1 : c.customs === false ? 0 : null },
];
...
function _compareSummary(list, scores) {
    let bi = 0;
    for (let i = 1; i < list.length; i++) if (scores[i] > scores[bi]) bi = i;
    const sorted = [...scores].sort((a, b) => b - a);
    if (!sorted[0]) return '';
    const margin = sorted[0] - (sorted[1] || 0);
    if (margin < 0.4) return t('cmp_close_call');
    const w = list[bi];
    const uniqMax = (f) => { const vs = list.map(f).filter(v => v != null); if (vs.length < 2) return false; const m = Math.max(...vs); return f(w) === m && vs.filter(v => v === m).length === 1; };
    const uniqMin = (f) => { const vs = list.map(f).filter(v => v != null); if (vs.length < 2) return false; const m = Math.min(...vs); return f(w) === m && vs.filter(v => v === m).length === 1; };
    const reasons = [];
...
function renderCompareModal() {
    ...
    const rows = [
        ...
        { label: t('spec_mileage'),   disp: c => c.mileage ? c.mileage.toLocaleString() + (en ? ' km' : ' კმ') : dash, val: c => c.mileage || null,                    better: 'low' },
        { label: t('filter_price'),   disp: c => c.price > 0 ? '$' + c.price.toLocaleString() : t('price_negotiable'), val: c => c.price > 0 ? c.price : null,          better: 'low' },
        { label: t('spec_engine'),    disp: c => c.engine ? c.engine + ' L' : dash },
        { label: t('spec_power'),     disp: c => c.power ? c.power + (en ? ' hp' : ' ც.ძ.') : dash,                    val: c => c.power || null,                       better: 'high' },
        { label: t('filter_fuel'),    disp: c => c.fuel ? escapeHtml(tval(c.fuel)) : dash },
        { label: t('filter_gearbox'), disp: c => c.gearbox ? escapeHtml(tval(c.gearbox)) : dash },
        { label: t('filter_drive'),   disp: c => c.drive ? escapeHtml(tval(c.drive)) : dash,                          val: c => c.drive ? _driveScore(c.drive) : null, better: 'high' },
        { label: t('filter_body'),    disp: c => c.body ? escapeHtml(tval(c.body)) : dash },
        { label: t('spec_customs'),   disp: c => c.customs === true ? t('cleared') : c.customs === false ? t('not_cleared') : dash, val: c => c.customs === true ? 1 : c.customs === false ? 0 : null, better: 'high' },
        { label: t('filter_location'),disp: c => c.location ? escapeHtml(locLabel(c.location)) : dash },
    ];
    ...
    const scoreRow = (row) => {
        ...
        return vals.map((v, ci) => {
            if (v == null) return '';
            if (v === best) { wins[ci]++; return 'cmp-best'; }
...
window.openCompare = function () {
    ...
    if (!modal) {
        modal = document.createElement('div'); modal.id = 'cmp-modal'; modal.className = 'cmp-modal';
...
window.closeCompare = function () {
    ...
    unlockScroll('cmp');
    if (_cmpOpener && typeof _cmpOpener.focus === 'function') { _cmpOpener.focus(); }
    _cmpOpener = null;
};
window._refreshCompareUI = function () {
    renderCompareBar();
    const modal = document.getElementById('cmp-modal');
    if (modal && modal.classList.contains('show')) { modal.querySelector('h3').textContent = t('compare_title'); renderCompareModal(); }
};
...
const _TR_LANGS = [['en', 'English'], ['ru', 'Русский'], ['kk', 'Қазақша']];
window.translateDesc = function (btn) {
    const block = btn.closest('.desc-block');
    const pop = block.querySelector('.tr-pop');
    if (!pop.hidden) { pop.hidden = true; btn.classList.remove('is-on'); return; }
...
function parseRoute(path) {
    ...
    if (path === '/' || path === '') return { view: 'search' };
    const m = path.match(_CAR_KEY_RE);
    return m ? { view: 'detail', key: m[1] } : { view: 'notfound' };
}
...
function renderDetail(car) {
    ...
    const dateLine = car.posted_date && car.posted_date.trim()
        ? t('posted_on', { date: escapeHtml(car.posted_date.trim()) })
        : t('scraped_on_note', { date: formatDate(car.created_at) });
    const carKey = `${car.source}-${car.source_id}`;
    _carData[carKey] = { key: carKey, title, price: car.price_amount, currency: car.price_currency, thumb: (car.image_urls && car.image_urls[0]) || '' };
    _carFull[carKey] = car;
...
window.dtSetMain = function (i) {
    ...
    if (img) {
        ...
        img.classList.remove('photo-anim'); void img.offsetWidth; img.classList.add('photo-anim');   // animate switch
    }
    ...
    if (c) c.textContent = t('photo_counter', { i: i + 1, n: _detailSlides.length });
...
window.goToCarDetail = function (key, ev) {
    ...
    history.pushState({ view: 'detail', key }, '', `/car/${key}`);
    showDetailView(key);
    window.scrollTo(0, 0);
    return false;
};
...
function _copyFeedback(btn) {
    ...
    setTimeout(() => { btn.innerHTML = orig; btn.classList.remove('is-copied'); }, 1300);
}
...
window.copyText = function (arg, maybeBtn) {
    let text, btn;
    if (arg && arg.nodeType === 1) { btn = arg; text = arg.getAttribute('data-copy') || ''; }
    else { text = arg; btn = maybeBtn; }
    if (navigator.clipboard && navigator.clipboard.writeText) {
...
const _carData = {};
const LS_CARS = 'cdb_saved', LS_SEARCHES = 'cdb_searches', LS_HIST = 'cdb_history', LS_VIEWED = 'cdb_viewed';
function lsGet(k) { try { const v = JSON.parse(localStorage.getItem(k)); return Array.isArray(v) ? v : []; } catch (_) { return []; } }
function lsSet(k, v) { try { localStorage.setItem(k, JSON.stringify(v)); } catch (_) {} }

function pushViewed(car) {
    const item = {
        key: `${car.source}-${car.source_id}`, title: carTitle(car),
        price: car.price_amount, currency: car.price_currency,
        thumb: (car.image_urls && car.image_urls[0]) || '',
    };
...
window.clearViewed = function () { lsSet(LS_VIEWED, []); renderSaved(); };

function getSavedCars() { return lsGet(LS_CARS); }
function isCarSaved(key) { return getSavedCars().some(c => c.key === key); }
window.toggleSave = function (key, btn) {
    let cars = getSavedCars();
    const had = cars.some(c => c.key === key);
    cars = had ? cars.filter(c => c.key !== key) : [(_carData[key] || { key: key, title: key }), ...cars].slice(0, 300);
    lsSet(LS_CARS, cars);
    if (btn && !had) { btn.classList.remove('pop'); void btn.offsetWidth; btn.classList.add('pop'); }
    refreshHearts();
    updateSavedCount();
};
...
function _searchParts(p) {
    ...
    if (p.drives && p.drives.length) out.push({ label: t('filter_drive'), value: p.drives.map(tval).join(', ') });
    if (p.locations && p.locations.length) out.push({ label: t('filter_location'), value: [...new Set(p.locations.map(locLabel))].join(', ') });
    if (p.customs_cleared === true) out.push({ label: t('spec_customs'), value: t('cleared') });
    if (p.customs_cleared === false) out.push({ label: t('spec_customs'), value: t('not_cleared') });
    if (p.sort) out.push({ label: t('filter_sort'), value: t('sort_' + p.sort) });
    return out;
}
function _payloadOf(item) {
    return (item && item.payload) || (item && item.q != null ? { query: item.q, sort: item.sort } : (typeof item === 'string' ? { query: item } : {}));
}
function _chipShort(item) {
    const parts = _searchParts(_payloadOf(item));
    if (!parts.length) return { text: (item && item.label) || t('save_search'), extra: 0 };
    return { text: parts[0].value, extra: parts.length - 1 };
}
...
window.saveCurrentSearch = function (btn) {
    ...
    if (exists) {
        ...
        s = [{ payload: payload, label: describeSearch(payload) }, ...s];
...
function _setMulti(id, values) {
    ...
    Array.from(sel.options).forEach(o => { o.selected = o.value ? o.value.split(_LOC_SEP).some(v => set.has(v)) : false; });
}
function applySearchPayload(p) {
    _setVal('search-input', p.query);
    _setVal('f-year-from', p.year_from); _setVal('f-year-to', p.year_to);
    _setVal('f-price-from', p.price_from); _setVal('f-price-to', p.price_to);
    _setVal('f-mileage-from', p.mileage_from); _setVal('f-mileage-to', p.mileage_to);
    _setMulti('f-brand', p.manufacturers);
    document.getElementById('f-brand').dispatchEvent(new Event('change', { bubbles: true }));   // rebuild model options
    ...
    refreshDropdowns(); updateFilterResets();
...
function _ensureSearchRoute() {
    if (parseRoute().view !== 'search') history.pushState({ view: 'search' }, '', '/');
}
function runSavedSearch(item) {
    closeSaved();
    _ensureSearchRoute();
    applySearchPayload((item && item.payload) || (item && item.q != null ? { query: item.q, sort: item.sort } : {}));
    doSearch();
}
function runHistory(item) {
    ...
    closeSaved();
    _ensureSearchRoute();
    const el = document.getElementById('search-input'); el.value = q; el.dispatchEvent(new Event('input')); doSearch();
}
...
function pushHistory() {
    ...
    if (Object.keys(payload).length === 0) return;
    const key = JSON.stringify(payload);
    const same = (x) => JSON.stringify((x && x.payload) || { query: x }) === key;
    let h = lsGet(LS_HIST).filter(x => !same(x));
    h.unshift({ payload, label: describeSearch(payload) });
...
function _saveHistItem(i) {
    const item = lsGet(LS_HIST)[i]; if (!item) return;
    const payload = (item && item.payload) || { query: String(item) };
    const key = JSON.stringify(payload);
    const s = lsGet(LS_SEARCHES);
    if (s.some(x => JSON.stringify(x.payload) === key)) return;
    lsSet(LS_SEARCHES, [{ payload, label: describeSearch(payload) }, ...s].slice(0, 40));
    updateSavedCount(); renderRecent();
}
...
function renderRecent() {
    ...
    if (!row) return;
    const allSaved = lsGet(LS_SEARCHES), allHist = lsGet(LS_HIST);
    if (!allSaved.length && !allHist.length) { row.innerHTML = ''; return; }
    const narrow = matchMedia('(max-width: 560px)').matches;
    const SCAP = narrow ? 2 : 4, HCAP = narrow ? 3 : 5;
    ...
    if (saved.length) {
        html += `<span class="recent-label">${t('saved_searches_h')}</span>` +
            saved.map((s, i) => _searchChip({ runKind: 'saved', delKind: 'saved-del', i, item: s, uid: 'sdr-s-' + i, saved: true })).join('');
    }
    if (hist.length) {
        html += `<span class="recent-label">${t('recent_h')}</span>` +
            hist.map((item, i) => _searchChip({ runKind: 'hist', delKind: 'hist-del', saveKind: 'hist-save', i, item, uid: 'sdr-h-' + i, saved: false })).join('') +
            `<button class="hist-chip recent-clear" data-kind="clear-all">✕ ${t('clear_all')}</button>`;
    }
    const total = allSaved.length + allHist.length, shown = saved.length + hist.length;
    ...
    row.querySelectorAll('.hist-chip[data-kind], .chip-del[data-kind], .chip-save[data-kind]').forEach((b) => {
        b.onclick = (e) => {
            ...
            switch (b.dataset.kind) {
                case 'saved': { const s = lsGet(LS_SEARCHES)[i]; if (s) runSavedSearch(s); break; }
                case 'saved-del': { const s = lsGet(LS_SEARCHES); s.splice(i, 1); lsSet(LS_SEARCHES, s); updateSavedCount(); renderRecent(); break; }
                case 'hist': { const item = lsGet(LS_HIST)[i]; if (item) runHistory(item); break; }
                case 'hist-save': _saveHistItem(i); break;
                case 'hist-del': { const h = lsGet(LS_HIST); h.splice(i, 1); lsSet(LS_HIST, h); renderRecent(); break; }
                case 'clear-all': lsSet(LS_HIST, []); renderRecent(); break;
                case 'more': _recentAll = true; renderRecent(); break;
                case 'less': _recentAll = false; renderRecent(); break;
            }
        };
    });
    _wireChipExpand(row);
}
window.clearHistory = function () { lsSet(LS_HIST, []); renderSaved(); renderRecent(); };
window.clearSavedCars = function () { lsSet(LS_CARS, []); updateSavedCount(); renderSaved(); };
window.clearSavedSearches = function () { lsSet(LS_SEARCHES, []); updateSavedCount(); renderSaved(); };
...
window.openSaved = function () { _savedOpener = document.activeElement; _drawerShown = {}; renderSaved(); document.getElementById('saved-overlay').classList.add('open'); document.getElementById('saved-drawer').classList.add('open'); document.body.classList.add('saved-open'); lockScroll('saved'); };
window.closeSaved = function () {
    document.getElementById('saved-overlay').classList.remove('open'); document.getElementById('saved-drawer').classList.remove('open'); document.body.classList.remove('saved-open'); unlockScroll('saved');
    if (_savedOpener && typeof _savedOpener.focus === 'function') _savedOpener.focus({ preventScroll: true });
    _savedOpener = null;
};

const EXPORT_KEYS = [LS_CARS, LS_SEARCHES, LS_HIST, LS_VIEWED, HIDDEN_KEY, CMPSAVE_KEY, 'cardb_lang'];
window.exportData = function () {
    const payload = { app: 'cardb', version: 1, exported: new Date().toISOString(), data: {} };
...
window.importData = function (input) {
    ...
    reader.onload = () => {
        let data = null;
        try { const p = JSON.parse(reader.result); data = p && p.data; } catch (_) {}
        if (!data || typeof data !== 'object') { showToast(t('import_error')); return; }
        let langChanged = false, applied = 0;
        ...
        if (!applied) { showToast(t('import_error')); return; }
        updateSavedCount();
        renderSaved();
...
function renderSaved() {
    const cars = getSavedCars(), searches = lsGet(LS_SEARCHES), hist = lsGet(LS_HIST);
    let h = '';
    ...
    body.innerHTML = h;
    body.querySelectorAll('[data-act="open"]').forEach(el => el.onclick = (e) => { goToCarDetail(el.dataset.key, e); closeSaved(); });
    body.querySelectorAll('[data-act="remove"]').forEach(el => el.onclick = () => { toggleSave(el.dataset.key); renderSaved(); });
    ...
    body.querySelectorAll('[data-act="unhide"]').forEach(el => el.onclick = (e) => { e.stopPropagation(); unhideCar(el.dataset.key); });
    ...
    body.querySelectorAll('.hist-chip[data-kind]').forEach(b => {
        b.onclick = () => {
            if (b.dataset.kind === 'search') { const s = lsGet(LS_SEARCHES)[+b.dataset.i]; if (s) runSavedSearch(s); }
            else { const q = lsGet(LS_HIST)[+b.dataset.i]; if (q) runHistory(q); }
        };
    });
    body.querySelectorAll('.chip-del').forEach((b) => {
        b.onclick = (e) => {
            ...
            const a = lsGet(lk); a.splice(+b.dataset.i, 1); lsSet(lk, a);
            updateSavedCount(); renderSaved(); renderRecent();
        };
    });
}
...
(function wireExtras() {
    const el = document.getElementById('search-input'), x = document.getElementById('clear-search');
    if (el && x) { const tog = () => x.classList.toggle('hidden', !el.value); el.addEventListener('input', tog); tog(); }
    updateSavedCount();
    renderRecent();
    document.addEventListener('keydown', (e) => {
        if (e.key !== 'Escape') return;
...
window.addEventListener('popstate', () => {
    ...
    if (document.querySelector('#saved-drawer.open')) closeSaved();
    const sb = document.getElementById('filter-sidebar');
    if (sb && sb.classList.contains('open')) { sb.classList.remove('open'); unlockScroll('sheet'); }
    const route = parseRoute();
    if (route.view === 'detail') { showDetailView(route.key); window.scrollTo(0, 0); }
    else if (route.view === 'notfound') showNotFound();
    else { showSearchView(); requestAnimationFrame(() => window.scrollTo(0, _searchScrollY)); }
});
...
function enhanceDropdowns() {
    ...
    const closeAll = () => {
        ...
        document.querySelectorAll('.cdd.open').forEach(d => {
            ...
            const b = d.querySelector('.cdd-btn'); if (b) { b.setAttribute('aria-expanded', 'false'); b.removeAttribute('aria-activedescendant'); }
        });
    };
    ...
    window.addEventListener('scroll', (e) => {
        ...
        const r = btn.getBoundingClientRect();
        if (r.bottom < 4 || r.top > window.innerHeight - 4) { closeAll(); return; }
...
function enhanceSelect(sel) {
    ...
    if (_lbl) { if (!_lbl.id) _lbl.id = _uid + '-lbl'; btn.setAttribute('aria-labelledby', _lbl.id); }

    const realSel = () => Array.from(sel.selectedOptions).filter(o => o.value);
    const anyLabel = () => { const a = Array.from(sel.options).find(o => !o.value); return a ? a.textContent : ''; };
    ...
    const _groupMembers = (gi) => { const out = []; for (let j = gi + 1; j < sel.options.length; j++) { const o = sel.options[j]; if (o.disabled) break; if (o.value) out.push(j); } return out; };
    const _groupState = (gi) => { const m = _groupMembers(gi); if (!m.length) return 'none'; const c = m.filter(j => sel.options[j].selected).length; return c === 0 ? 'none' : c === m.length ? 'all' : 'some'; };
    const _allValueIdx = () => { const out = []; for (let j = 0; j < sel.options.length; j++) { const o = sel.options[j]; if (o.value && !o.disabled) out.push(j); } return out; };
    const _allState = () => { const a = _allValueIdx(); if (!a.length) return 'none'; const c = a.filter(j => sel.options[j].selected).length; return c === 0 ? 'none' : c === a.length ? 'all' : 'some'; };
    ...
    const _syncOptRow = (i) => { const el = panel.querySelector(`.cdd-opt[data-i="${i}"]`); if (!el) return; const on = sel.options[i].selected; el.classList.toggle('on', on); el.setAttribute('aria-selected', on ? 'true' : 'false'); const box = el.querySelector('.cdd-box'); if (box) { box.classList.remove('mixed'); box.classList.toggle('on', on); } };
    const _syncGroupChecks = () => { panel.querySelectorAll('[data-groupcheck]').forEach(gc => { const st = _groupState(+gc.dataset.groupcheck); gc.classList.toggle('on', st === 'all'); gc.classList.toggle('mixed', st === 'some'); gc.setAttribute('aria-checked', st === 'all' ? 'true' : st === 'some' ? 'mixed' : 'false'); }); };
    const _syncSelectAll = () => { const sa = panel.querySelector('[data-selectall] .cdd-box'); if (!sa) return; const st = _allState(); sa.classList.toggle('on', st === 'all'); sa.classList.toggle('mixed', st === 'some'); };
    ...
    panel.addEventListener('click', (e) => {
        ...
        if (gcheck) {
            ...
            members.forEach(_syncOptRow); _syncGroupChecks(); _syncSelectAll(); updateLabel();
            return;
        }
        ...
        if (sa) {
            ...
            _clearAny();
            sel.dispatchEvent(new Event('change', { bubbles: true }));
            const rowsEl = panel.querySelector('.cdd-rows'); if (rowsEl) rowsEl.innerHTML = rowsHtml();
            updateLabel();
            return;
        }
        ...
        if (multi) {
            ...
            if (!o.value) {
                ...
                o.selected = !o.selected; _clearAny();
                sel.dispatchEvent(new Event('change', { bubbles: true }));
                _syncOptRow(i); _syncGroupChecks(); _syncSelectAll();
            }
            updateLabel();
        } else {
            sel.selectedIndex = i;
    ...
    const setActive = (idx) => {
        const rows = rowEls();
        if (!rows.length) { _activeIdx = -1; const fo = focusOwner(); if (fo) fo.removeAttribute('aria-activedescendant'); return; }
        ...
        const fo = focusOwner(); if (fo) fo.setAttribute('aria-activedescendant', el.id);
        el.scrollIntoView({ block: 'nearest' });
    };
    const openPanel = (at) => {
        if (sel.disabled) return;
        document.querySelectorAll('.cdd.open').forEach(d => { d.classList.remove('open'); const b = d.querySelector('.cdd-btn'); if (b) b.setAttribute('aria-expanded', 'false'); });
        _filter = ''; render(); wrap.classList.add('open'); btn.setAttribute('aria-expanded', 'true');
        document.body.classList.add('cdd-open');   // lifts the sidebar's stacking context above the results
        _placeCddPanel(btn, panel); panel.scrollTop = 0;
        const fo = focusOwner(); if (fo && fo !== btn) fo.focus();
    ...
    const closePanel = (focusBtn) => {
        ...
        document.body.classList.remove('cdd-open');
        const fo = focusOwner(); if (fo) fo.removeAttribute('aria-activedescendant');
        _activeIdx = -1;
        if (focusBtn) btn.focus();
    };
    const activateRow = () => {
        const rows = rowEls(); const el = rows[_activeIdx]; if (!el) return;
    ...
    const onKey = (e) => {
        ...
        if (!wrap.classList.contains('open')) {
            if (e.key === 'ArrowDown' || e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openPanel('first'); }
            else if (e.key === 'ArrowUp') { e.preventDefault(); openPanel('last'); }
            return;
        }
        switch (e.key) {
            case 'ArrowDown': e.preventDefault(); setActive(_activeIdx + 1); break;
            case 'ArrowUp':   e.preventDefault(); setActive(_activeIdx - 1); break;
            case 'Home':      e.preventDefault(); setActive(0); break;
            case 'End':       e.preventDefault(); setActive(rowEls().length - 1); break;
            case 'Enter':     e.preventDefault(); activateRow(); break;
            case ' ':
                if (e.target.classList && e.target.classList.contains('cdd-search')) return;  // let space type
                e.preventDefault(); activateRow(); break;
            case 'Escape':    e.preventDefault(); closePanel(true); break;
            case 'Tab':       closePanel(false); break;
        }
    };
    wrap.addEventListener('keydown', onKey);
    sel.addEventListener('change', () => { wrap.classList.contains('open') ? updateLabel() : render(); });
    new MutationObserver((muts) => { if (muts.some(m => m.type === 'childList')) _expanded.clear(); render(); }).observe(sel, { childList: true, attributes: true, attributeFilter: ['disabled'] });
    sel._cdd = { render };
    render();
}
...
(function bootOverlay() {
    ...
    if (!ov) return;
    let done = false;
    const hide = () => { if (done) return; done = true; ov.classList.add('hidden'); setTimeout(() => ov.remove(), 500); };
...
(function restoreFromUrl() {
    ...
    const page = p.page; delete p.page;
